...
const NocturneBackup = (() => {
  ...
  function normalizeState(input) {
    ...
    for (const [key, record] of Object.entries(input.days)) {
      if (!date(key) || !object(record) || !object(record.tasks) || !Number.isInteger(record.water) || record.water < 0 || record.water > 6) fail('This backup contains an invalid daily record.');
      const tasks = {};
